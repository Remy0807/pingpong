...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const bestWinRate = [...activePlayers].sort((a, b) => b.winRate - a.winRate)[0];
    ...
    <div className="flex flex-col gap-8">
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-5">
          <span className="text-xs uppercase tracking-widest text-axoft-300">Beste winrate</span>
          <h3 className="mt-3 text-xl font-semibold text-white">
            {stats.bestWinRate ? stats.bestWinRate.player.name : "Nog onbekend"}
          </h3>
          <p className="mt-1 text-sm text-slate-300">
            {stats.bestWinRate
              ? `${Math.round(stats.bestWinRate.winRate * 100)}% over ${stats.bestWinRate.matches} potjes`
              : "Er zijn eerst gespeelde potjes nodig."}
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-5">
          <span className="text-xs uppercase tracking-widest text-axoft-300">Meeste potjes</span>
          <h3 className="mt-3 text-xl font-semibold text-white">
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-5">
          <span className="text-xs uppercase tracking-widest text-axoft-300">Beste saldo</span>
          <h3 className="mt-3 text-xl font-semibold text-white">
            {stats.bestDifferential ? stats.bestDifferential.player.name : "Nog onbekend"}
          </h3>
          <p className="mt-1 text-sm text-slate-300">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,60%),minmax(0,40%)]">
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-white">Laatste resultaten</h2>
