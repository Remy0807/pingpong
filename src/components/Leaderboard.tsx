  ...
  if (!players.length) {
    return (
      <div className="glass-card rounded-xl p-6 text-center text-slate-400">
        Nog geen statistieken beschikbaar. Voeg resultaten toe om het klassement te vullen.
      </div>
    );
  }
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {topThree.map((entry, index) => (
          ...
          >
            ...
            <h3 className="mt-4 text-xl font-semibold text-white">{entry.player.name}</h3>
            <dl className="mt-6 grid grid-cols-2 gap-4 text-sm text-slate-300">
              <div>
                <dt className="text-xs uppercase tracking-widest text-axoft-200/70">Potjes</dt>
                <dd className="text-lg font-semibold text-white">
                  {numberFormatter.format(entry.matches)}
                </dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-widest text-axoft-200/70">Saldo</dt>
                <dd className="text-lg font-semibold text-white">
          ...
          {sorted.map((entry) => (
            ...
            >
              <header className="flex items-center justify-between gap-3">
                <h3 className="text-base font-semibold text-white">{entry.player.name}</h3>
                <span className="rounded-full bg-axoft-500/15 px-3 py-1 text-xs font-medium text-axoft-200">
                  {percentageFormatter.format(entry.winRate)}
                </span>
              </header>
              <dl className="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-300">
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Potjes</dt>
                  <dd className="text-sm font-semibold text-white">{entry.matches}</dd>
                </div>
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Saldo</dt>
                  <dd
                ...
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Gewonnen</dt>
                  <dd className="text-sm font-semibold text-emerald-300">{entry.wins}</dd>
                </div>
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Verloren</dt>
                  <dd className="text-sm font-semibold text-rose-300">{entry.losses}</dd>
                </div>
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Punten +</dt>
                  <dd className="text-sm font-semibold text-slate-100">{entry.pointsFor}</dd>
                </div>
                <div>
                  <dt className="uppercase tracking-widest text-slate-500">Punten -</dt>
                  <dd className="text-sm font-semibold text-slate-100">{entry.pointsAgainst}</dd>
                </div>
              </dl>
            </article>
          ))}
            ...
            <tbody>
              {sorted.map((entry) => (
                <tr key={entry.player.id} className="border-t border-white/5">
                  <td className="px-4 py-3 font-semibold text-white">{entry.player.name}</td>
                  ...
                  <td className="px-4 py-3 text-slate-200">{entry.pointsAgainst}</td>
                  <td
