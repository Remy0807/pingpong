...
export function SeasonOverview({ seasons, currentSeasonId }: SeasonOverviewProps) {
  ...
  const currentSeason =
    seasons.find((season) => season.id === currentSeasonId) ?? seasons[0] ?? null;
  const previousSeasons = seasons.filter((season) => season.id !== currentSeason?.id);
      ...
      {currentSeason ? (
        <div className="glass-card rounded-2xl border border-white/10 p-6">
          <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest text-axoft-200">Huidig seizoen</p>
              <h3 className="text-2xl font-semibold text-white">{currentSeason.name}</h3>
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/60 px-4 py-3 text-sm text-slate-200">
              <p className="text-xs uppercase tracking-widest text-slate-400">Wedstrijden</p>
              <p className="text-lg font-semibold text-white">{currentSeason.matches}</p>
              <p className="text-xs text-slate-500">totaal deze maand</p>
            </div>
          </header>

          <div className="mt-6 space-y-2">
            <p className="text-xs uppercase tracking-widest text-axoft-200">Top 3</p>
            <div className="grid gap-3 md:grid-cols-3">
              {currentSeason.standings.slice(0, 3).map((standing, index) => (
                ...
                >
                  <p className="text-xs uppercase tracking-widest text-axoft-300">#{index + 1}</p>
                  <p className="mt-1 text-base font-semibold text-white">
                    {standing.player.name}
                  </p>
                  <p className="text-xs text-slate-400">
                    {standing.wins}–{standing.losses} ({percentage(standing.winRate)})
                  </p>
                  <p
                    className={`mt-2 text-xs font-semibold ${
                      standing.pointDifferential >= 0 ? "text-emerald-300" : "text-rose-300"
                    }`}
                  >
        ...
        <div className="glass-card rounded-2xl border border-white/10 p-6">
          ...
          >
            {previousSeasons.map((season) => (
              ...
              >
                <div>
                  <p className="text-base font-semibold text-white">{season.name}</p>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="rounded-lg border border-white/10 bg-slate-900/50 px-3 py-2 text-xs">
                    Wedstrijden: <span className="font-semibold">{season.matches}</span>
                  </div>
                  <div className="rounded-lg border border-white/10 bg-slate-900/50 px-3 py-2 text-xs">
                    Kampioen:{" "}
